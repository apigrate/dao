/*
  Copyright 2018-2022 Apigrate, LLC
 ...
 */
 async function saveAll(req, res, next){
  try{
    ...
    debug(`${shared.length} entities intersect`);
    let delete_these = _.xorBy(existing, shared, dbi.comparison );
    debug(`...of which ${shared.length - delete_these.length} will be updated and ${delete_these.length} will be deleted.`);
    let create_these = _.xorBy(dbi.toSave, shared, dbi.comparison );
    debug(`${create_these.length} will be created.`);
    ...
    //Updates
    let sharedids = shared.map(s=>s.id);
    let update_these = dbi.toSave.filter(save=>{ return sharedids.includes( save.id );});
    let updated = await Promise.all( update_these.map( function(entity){ return dbi.dao.update(entity); } ) );
