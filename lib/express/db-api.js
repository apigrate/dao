 ...
 * @param {array} allowed_search_term_fields database fields that can be queried by a search term - MUST be a subset of allowed_query_fields.
 * 
 ...
 */
function parseQueryOptions(allowed_query_fields, default_orderby, default_limit, allowed_search_term_fields){
  ...
  function parseQueryOptionsFromObject(obj){
    ...
    for (key in query) {
      ...
      if (key === 'search_term' && allowed_search_term_fields && allowed_search_term_fields.length>0){
        criteria = new CriteriaHelper();
        criteria.andGroup();
        for(let field_name of allowed_search_term_fields){
  ...
  return function(req, res, next){
    ...
    if(result){
      ...
      console.log(res.locals.criteria);
