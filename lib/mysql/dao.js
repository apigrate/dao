/*
  Copyright 2018-2025 Apigrate, LLC
...
class Dao{
  constructor (table, entity, opts, pool){ 
    ...
    // If a table name matching the suppression list is found, then logging will be suppressed.
    // Tables must be specified in lowercase.
    if(process.env.DB_SUPPRESS_LOGGING_TABLES){
      const table_names = process.env.DB_SUPPRESS_LOGGING_TABLES.split(',');
      if(table_names.includes(this.table.toLowerCase())){
   ...
   * @param {boolean} opts.suppressLogging suppresses logging of the SQL command and parameters
   * @return {Promise} the result of the SQL command
   */
  async sqlCommand(sql, parms, opts){
    let self = this;
    
    return new Promise(function(resolve, reject){
      if(!sql.includes("SHOW COLUMNS")){ //suppress metadata logging.
        if(self.suppressLogging || (opts && opts.suppressLogging)){
          // debug(`  sql: <suppressed>`); //suppress ALL logging
        } else {
          debug(`  sql: ${sql}`);
          if(parms && parms.length>0){
            verbose(`  parms: ${JSON.stringify(parms)}`);
          }
        }
      }
  ...
  */
  async get(id, opts){
    ...
    try{
      ...
      let results = await this.sqlCommand(sql, [id], opts);
  ...
  */
  async all(opts){
    try {
      ...
      return await this.sqlCommand(sql, [], opts);
  ...
  */
  async query(query, opts){
    try{
      ...
      let results = await this.sqlCommand(sql, parms, opts);
  ...
  */
  async one(query, opts){
    try {
      ...
      var entityResult = null;
      let result = await this.query(query,opts)
      if(result.length>0){
  ...
  /**
    ...
    @param opts options for logging suppression etc (optional).
    @example
    {
      columns: ['column1', 'column2', 'column3'], //to be returned
      orderBy: ['+column_name','-column_name'],
  ...
  */
  async create(save, opts){
    try {
      ...
      let results = await this.sqlCommand(sql, parms, opts);
      if(this.suppressLogging || (opts && opts.suppressLogging)){
  ...
  */
  async updateMatching(save, criteria, opts){
    try {
      if(!criteria || Object.keys(criteria).length === 0) throw new Error(`Entire table updates are not permitted.`);
      debug( this.entity + ' update...' );
  ...
  */
  ...
  /**
    ...
    @param opts (optional) options for logging suppression, etc.
    @return a promise bearing the incoming object with an _affectedRows attribute added.
  */
  async deleteOne(toDelete, opts){
    try {
  ...
  * @param id (required) the id of the entity to delete.
  * @param opts options for logging suppression etc (optional).
  ...
  */
  async deleteMatching(criteria, opts){
    try {
  ...
  */
  async deleteWhere(where, parms, opts){
    try {
